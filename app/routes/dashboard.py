...
def project_detail(project_id):
    ...
    # Get recent content
    recent_content = Content.query.filter_by(project_id=project.id).order_by(Content.created_at.desc()).limit(10).all()
    
    # Get stats
    total_content = Content.query.filter_by(project_id=project.id).count()
    approved_content = Content.query.filter_by(project_id=project.id, status='approved').count()
    rejected_content = Content.query.filter_by(project_id=project.id, status='rejected').count()
    flagged_content = Content.query.filter_by(project_id=project.id, status='flagged').count()
...
def project_rules(project_id):
    ...
    if not project.is_member(current_user.id):
        flash('You do not have access to this project', 'error')
        return redirect(url_for('dashboard.projects'))
    rules = ModerationRule.query.filter_by(project_id=project.id).order_by(ModerationRule.priority.desc()).all()
...
def update_rule(project_id, rule_id):
    ...
    rule = ModerationRule.query.filter_by(id=rule_id, project_id=project.id).first_or_404()
...
def get_content_details(project_id, content_id):
    ...
    if not project.is_member(current_user.id):
        return jsonify({'success': False, 'error': 'Access denied'}), 403
    content = Content.query.filter_by(id=content_id, project_id=project.id).first_or_404()
...
def toggle_api_key(project_id, key_id):
    ...
    if not project.is_member(current_user.id):
        return jsonify({'success': False, 'error': 'Access denied'}), 403
    api_key = APIKey.query.filter_by(id=key_id, project_id=project.id).first_or_404()
...
def accept_invitation(token):
    ...
    flash(f'You have successfully joined the project "{project.name}"', 'success')
    return redirect(url_for('dashboard.project_detail', project_id=project.id))
