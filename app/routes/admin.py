...
def user_detail(user_id):
    ...
    total_rules = sum(len(project.moderation_rules) for project in user_projects)
...
def create_user():
    ...
    try:
        ...
        flash(f'User {username} created successfully as {role} ({status}).', 'success')
...
def delete_user(user_id):
    ...
    try:
        ...
        if user_projects:
            ...
            flash(f'Cannot delete user {username}. They have {project_count} project(s) that must be deleted first.', 'error')
...
def logs():
    ...
    recent_content = Content.query.order_by(Content.created_at.desc()).limit(50).all()
    recent_moderations = ModerationResult.query.order_by(ModerationResult.created_at.desc()).limit(50).all()
