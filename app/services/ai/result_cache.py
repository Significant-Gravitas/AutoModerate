...
class ResultCache:
        ...
        if cache_key in ResultCache._shared_cache:
            ...
            if time.time() - cached_data['timestamp'] < ResultCache._shared_cache_ttl:
                ...
                if current_app.logger.level <= 10:
                    current_app.logger.info(f"Cache EXPIRED for key: {cache_key[:8]}...")
        return None
    ...
    def cache_result(self, cache_key, result):
        ...
        if cache_size > 2000:  # Increased max cache size
            ...
            for key, data in ResultCache._shared_cache.items():
                if current_time - data['timestamp'] >= ResultCache._shared_cache_ttl * 0.5:  # Remove entries at 50% of TTL
                    entries_to_remove.append(key)
            
            # If not enough expired entries, remove oldest ones
            if len(entries_to_remove) < len(ResultCache._shared_cache) * 0.25:
                sorted_keys = sorted(ResultCache._shared_cache.keys(), key=lambda k: ResultCache._shared_cache[k]['timestamp'])
                entries_to_remove.extend(sorted_keys[:max(500, len(ResultCache._shared_cache) // 4)])
    ...
    def _get_oldest_entry_age(self):
        ...
        current_time = time.time()
        oldest_timestamp = min(data['timestamp'] for data in ResultCache._shared_cache.values())
