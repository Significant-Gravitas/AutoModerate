...
class OpenAIService:
    ...
    def moderate_content(self, content, content_type='text', custom_prompt=None):
        ...
        try:
            ...
            # This ensures AI rules are evaluated based on their specific criteria
            if custom_prompt:
    ...
    def _analyze_with_custom_prompt(self, content, custom_prompt):
        ...
        try:
            ...
            # Check cache first
            cache_key = self._generate_cache_key(content, custom_prompt)
...
{{
    ...
    \"confidence\": 0.85
            ...
            current_app.logger.info("Running OpenAI custom prompt analysis...")
            ...
            current_app.logger.info("OpenAI custom prompt analysis completed successfully")
    ...
    @classmethod
    def _generate_cache_key(cls, content, custom_prompt):
        """
    ...
    @classmethod
    def _cache_result(cls, cache_key, result):
        """
        ...
        if len(cls._cache) > 1000:  # Max 1000 cached entries
    ...
    def _run_baseline_moderation(self, content):
        ...
        This catches obvious violations like explicit sexual content, violence, etc.
        """
        try:
            current_app.logger.info("Running OpenAI baseline moderation API...")
            
            # Use OpenAI's moderation endpoint
            response = self.client.moderations.create(input=content)
            result = response.results[0]
            
            current_app.logger.info("OpenAI baseline moderation completed")
    ...
    def _run_enhanced_default_moderation(self, content):
        ...
        This is more thorough than the baseline and catches edge cases
        """
        try:
            # Check cache first
            cache_key = self._generate_cache_key(content, "enhanced_default")
            cached_result = self._get_cached_result(cache_key)
            if cached_result:
                return cached_result
...
{{
    ...
    \"confidence\": 0.95
            ...
            current_app.logger.info("Running enhanced default moderation...")
            response = self.client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=[
                    {"role": "system", "content": system_message},
            ...
            current_app.logger.info("Enhanced default moderation completed")
